 ...
 */
export const getBottomStyles = ({ trigger, tooltip, offset, boundary }) => {
  ...
  if (leftTooltip < boundary.left) {
    ...
    console.log(leftTooltip, boundary.left)
    translateX = Math.abs(leftTooltip) + boundary.left;
 ...
 */
export const getTopStyles = ({ trigger, tooltip, offset, boundary }) => {
  const topArrow =
    trigger.top -
    offset -
    ARROW_HYPOTENUSE +
